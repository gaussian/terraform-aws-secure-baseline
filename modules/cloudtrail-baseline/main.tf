...
data "aws_iam_policy_document" "cloudtrail_key_policy" {
  ...
  statement {
    sid = "Allow CloudTrail (and CloudWatch/EventBridge) to send notifications to the encrypted SNS topic"
    principals {
      type        = "Service"
      identifiers = ["cloudtrail.amazonaws.com", "cloudwatch.amazonaws.com", "events.amazonaws.com"]
    }
    actions = [
      "kms:GenerateDataKey*",
      "kms:Decrypt"

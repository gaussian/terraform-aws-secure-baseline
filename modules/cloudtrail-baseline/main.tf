...
data "aws_iam_policy_document" "cloudtrail_key_policy" {
  ...
  statement {
    sid = "Allow CloudTrail (and CloudWatch/EventBridge/SES) to send notifications to the encrypted SNS topic"
    principals {
      type        = "Service"
      identifiers = ["cloudtrail.amazonaws.com", "cloudwatch.amazonaws.com", "events.amazonaws.com", "ses.amazonaws.com"]
    }
    actions = [
      "kms:GenerateDataKey*",
      "kms:Decrypt"
